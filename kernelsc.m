function [k1,k2,err,Kuus,Kuvs,Kvus,Kvvs] = kernelsc(lam,mu,c1,c2,p,r,xu,xv,N)
...
KE1 = L*diff(Kuu,x) + diff(Kuu,z)*subs(L,x,z) ... 
   + Kuu*subs(diff(L,x),x,z) + Kuv*subs(TH,x,z);
KE2 = L*diff(Kuv,x) - diff(Kuv,z)*subs(M,x,z) ... 
   - Kuv*subs(diff(M,x),x,z) + Kuu*subs(W,x,z);
KE3 = M*diff(Kvu,x) - diff(Kvu,z)*subs(L,x,z) ...
  - Kvu*subs(diff(L,x),x,z) - Kvv*subs(TH,x,z);
KE4 = M*diff(Kvv,x) + diff(Kvv,z)*subs(M,x,z) ...
  + Kvv*subs(diff(M,x),x,z) - Kvu*subs(W,x,z);
...
Kvus1 = subs(Kvus,x,1);
Kvvs1 = subs(Kvvs,x,1);
k1 = double(subs(Kvus1,z,xu) - r*subs(Kuus1,z,xu));
k2 = double(subs(Kvvs1,z,xv) - r*subs(Kuvs1,z,xv));
end