function [p1,p2,err,Puus,Puvs,Pvus,Pvvs] = kernelso(lam,mu,c1,c2,p,xu,xv,N)
...
KE1 = L*diff(Puu,x) + diff(Puu,z)*subs(L,x,z) ... 
   + Puu*subs(diff(L,x),x,z) + Pvu*W;
KE2 = L*diff(Puv,x) - diff(Puv,z)*subs(M,x,z) ... 
   - Puv*subs(diff(M,x),x,z) + Pvv*W;
KE3 = M*diff(Pvu,x) - diff(Pvu,z)*subs(L,x,z) ...
  - Pvu*subs(diff(L,x),x,z) - Puu*TH;
KE4 = M*diff(Pvv,x) + diff(Pvv,z)*subs(M,x,z) ...
  + Pvv*subs(diff(M,x),x,z) - Puv*TH;
BC1 = subs(Puu,x,0) - Q*subs(Pvu,x,0);
BC2 = (L+M)*subs(Puv,z,x) - W;
BC3 = (L+M)*subs(Pvu,z,x) + TH;
BC4 = subs(Puv,x,0) - Q*subs(Pvv,x,0);
...
P1s1 = subs(Puus,z,1);
P2s1 = subs(Pvus,z,1);
p1 = double(-lam*subs(P1s1,x,xu))';
p2 = double(-lam*subs(P2s1,x,xv))';
end